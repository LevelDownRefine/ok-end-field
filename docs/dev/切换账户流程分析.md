...
**定义位置：** [`src/tasks/BaseEfTask.py`](../../src/tasks/BaseEfTask.py) `::` `BaseEfTask.iter_multi_account_context`  
...
iter_multi_account_context(repeat_times, empty_accounts_message, ...)
  ...
  │   └─ [src/tasks/account/account_mixin.py](../../src/tasks/account/account_mixin.py)
  │       └─ resolve_account_id()        → 本项目
  │           └─ [src/tasks/account/account_scope_store.py](../../src/tasks/account/account_scope_store.py)
  │
  ├─ self.set_current_account(username, account_id)   → 本项目
  │   └─ [src/tasks/BaseEfTask.py](../../src/tasks/BaseEfTask.py)
  │       └─ _bind_account_aware_config_get()          → 本项目
  │           └─ [src/tasks/mixin/account_override_mixin.py](../../src/tasks/mixin/account_override_mixin.py)
  │
  └─ self.login_flow(username)           → 本项目
      └─ [src/tasks/mixin/login_mixin.py](../../src/tasks/mixin/login_mixin.py)
```
...
login_flow(username)
  ...
  ├─ self.ensure_main()                        → 本项目封装 [game_flow_mixin.py]
  │   └─ 最终 → 框架 BaseTask
  ├─ self.find_one(fL.main_out)                → 本项目封装 [runtime_mixin.py]
  │   └─ 最终 → 框架 BaseTask.find_one()
  ├─ self.click(result)                        → 本项目封装 [runtime_mixin.py]
  │   └─ → 框架 BaseTask.click()
  │       └─ → EfInteraction.click()           ✅ 本项目重写 [EfInteraction.py]
  │
  │  【阶段2：找登出按钮并登出】
  ├─ self.find_feature(fL.logout)              → 本项目封装 [runtime_mixin.py]
  ...
  │
  │  【阶段3：切换到目标账号】
  ├─ self.click_text(re.compile("最近"))        → ✅ 本项目实现 [login_mixin.py]
  ├─ self.click_text(re.compile(username[-4:])) → ✅ 本项目实现 [login_mixin.py]
  ├─ self.click_text("登录")                    → ✅ 本项目实现 [login_mixin.py]
  │
  │  【阶段4：确认登录成功】
  └─ self._confirm_logged_in()                 → 本项目 [login_mixin.py]
...
click_text(match, box, need_wait_disappear, success_match)
  │
  │  【截图】
  ├─ self.login_ocr(match, box, need_active=False)    → 本项目 [game_flow_mixin.py]
  │   │
  │   ├─ self.login_screenshot()                      → ✅ 本项目 [game_flow_mixin.py]
  │   │   └─ capture_window_by_screen(self.hwnd.hwnd) → ✅ 本项目 [src/image/login_screenshot.py]
  ...
  │
  │  【点击】
  └─ run_at_window_pos(self.hwnd.hwnd, pyautogui.click, x, y)   → ✅ 本项目 [Mouse.py]
       │
...
**定义位置：** [`src/tasks/mixin/game_flow_mixin.py`](../../src/tasks/mixin/game_flow_mixin.py) `::` `GameFlowMixin.login_screenshot`  
**来源：** ✅ **本项目独立实现**

```
login_screenshot(need_active=True)
  │
  ├─ self.active_and_send_mouse_delta()    → 本项目 [Mouse.py]
  │
  └─ capture_window_by_screen(self.hwnd.hwnd)  → ✅ 本项目 [src/image/login_screenshot.py]
       │
...
**定义位置：** [`src/tasks/mixin/game_flow_mixin.py`](../../src/tasks/mixin/game_flow_mixin.py) `::` `GameFlowMixin.login_ocr`  
**来源：** 🔀 混合（自定义截图 + 框架 OCR 引擎）

```
login_ocr(match, box, ...)
  │
  ├─ self.login_screenshot()    → ✅ 本项目（自定义截图源）
  │
  └─ super().ocr(frame=img, ...)  → 框架 BaseTask.ocr()
       │                           [.venv/Lib/site-packages/ok/task/task.py]
       └─ self.ocr_fun(lib)()
...
**定义位置：** [`src/tasks/mixin/runtime_mixin.py`](../../src/tasks/mixin/runtime_mixin.py) `::` `RuntimeMixin.find_feature` / `RuntimeMixin.find_one`  
**来源：** 🔀 本项目封装 + 框架引擎
...
**定义位置：** [`src/tasks/mixin/runtime_mixin.py`](../../src/tasks/mixin/runtime_mixin.py) `::` `RuntimeMixin.click`  
**来源：** 🔀 本项目封装 + 本项目 `EfInteraction`

```
click(x, y, ...)
  │
  ├─ self.find_danger()        → 本项目 [runtime_mixin.py]（危险态检测）
  │
  └─ super().click(...)        → 框架 BaseTask.click()
       │                        [.venv/Lib/site-packages/ok/task/task.py]
       └─ self.executor.interaction.click(x, y, ...)
            │
            └─ EfInteraction.click()  → ✅ 本项目 [src/interaction/EfInteraction.py]
                 │
...
**定义位置：** [`src/interaction/EfInteraction.py`](../../src/interaction/EfInteraction.py) `::` `EfInteraction.click`  
...
**定义位置：** [`src/tasks/mixin/account_override_mixin.py`](../../src/tasks/mixin/account_override_mixin.py)  
**来源：** ✅ **本项目完全自实现**
...
_bind_account_aware_config_get()
  ...
  └─ 猴子补丁：
       ...
       _patched_get(key, default):
         ...
         │   └─ config.get("多账户独立配置", False)
         │
         ├─ account_overrides = get_account_task_overrides(
         │       current_account_id, task_name)       → 本项目 [account_scope_store.py]
         │
...
| 函数 | 行号 | 职责 |
|------|------|------|
| `get_store_path()` | 21 | 返回 `configs/account_scoped_overrides.json` 路径 |
| `parse_account_list_text()` | 69 | 解析账号列表文本（兼容 `账号,密码` 格式） |
| `_normalize()` | 211 | 数据清洗：注册表标准化 + 账号映射标准化 |
| `load_overrides()` | 290 | 加载覆盖文件（带 mtime 缓存 + 线程锁） |
| `save_overrides()` | 318 | 保存覆盖文件 |
| `sync_account_list_text()` | 335 | 同步账号列表 → 注册表 ID 映射 |
| `resolve_account_id()` | 342 | 用户名 → account_id 解析 |
| `get_account_task_overrides()` | 358 | 按 account_id + task_name 读取覆盖 |
| `set_account_task_overrides()` | 406 | 写入覆盖值 |
| `_find_account_id_by_username()` | 142 | 在注册表中按用户名查找 ID |
| `_generate_account_id()` | 170 | `uuid4` 生成 `acc_xxxxxxxxxxxx` 格式 ID |
...
| 功能 | 文件 | 说明 |
|------|------|------|
| **账号 ID 注册表** | [`account_scope_store.py`](../../src/tasks/account/account_scope_store.py) | `acc_uuid` 生成、用户名 ↔ ID 映射、`aliases` 管理 |
| **账号覆盖存储** | [`account_scope_store.py`](../../src/tasks/account/account_scope_store.py) | JSON 文件读写、mtime 缓存、线程锁 |
| **运行时覆盖注入** | [`account_override_mixin.py`](../../src/tasks/mixin/account_override_mixin.py) | 猴子补丁 `config.get()` 实现 AOP 切面 |
| **多账号配置项声明** | [`account_mixin.py`](../../src/tasks/account/account_mixin.py) | "多账户模式"、"多账户独立配置"、"账号列表" |
| **账号列表解析** | [`account_mixin.py`](../../src/tasks/account/account_mixin.py) | `get_account_list()` + `resolve_account_id()` |
| **多账号循环执行** | [`BaseEfTask.py`](../../src/tasks/BaseEfTask.py) | `iter_multi_account_context()` 生成器 |
| **登出→切账号→登录** | [`login_mixin.py`](../../src/tasks/mixin/login_mixin.py) | 整个 `login_flow()` 流程 |
| **OCR 文字点击** | [`login_mixin.py`](../../src/tasks/mixin/login_mixin.py) | `click_text()` — 自定义截图 + pyautogui 点击 |
| **自定义截图** | [`login_screenshot.py`](../../src/image/login_screenshot.py) | `capture_window_by_screen()` — pyautogui + 裁剪 |
| **自定义鼠标点击** | [`EfInteraction.py`](../../src/interaction/EfInteraction.py) | 继承框架，重写 `click()` 添加窗口激活逻辑 |
| **GUI 覆盖编辑器** | [`AccountConfigTab.py`](../../src/gui/AccountConfigTab.py) | `render_task_editor()`、`_build_virtual_config()`、覆盖 CRUD |
| **覆盖编辑虚拟配置** | [`AccountConfigTab.py`](../../src/gui/AccountConfigTab.py) | `InMemoryConfig` 包装器，`_coerce_like()` 类型转换 |
