...
(function () {
    ...
    function clickLocationSyncOpen() {
        ...
        try {
            ...
            if (!row) {
                console.log('[坐标转发] 未找到同步开关');
                return false;
            }

            const onBtn = row.querySelector(
                '[class*="PointSwitch__ToggleBtn"]'
            );

            if (!onBtn) {
                console.log('[坐标转发] 未找到开启按钮');
                return false;
            }

            console.log('[坐标转发] 自动开启位置同步');
    ...
    WebSocket.prototype.addEventListener = function (type, listener, options) {
        ...
        if (
            type === 'message' &&
            this.url &&
            this.url.includes('ws.skport.com/ws/v1/game/endfield/map')
        ) {
